...
import com.pluralsight.conference.controller.util.OnPasswordResetEvent;
...
public class PasswordController {
    ...
    @Autowired
    private ApplicationEventPublisher eventPublisher; //provided by spring
    ...
    public String sendEmailToReset(@Valid @ModelAttribute("password") Password password, BindingResult result) {
        // check for errors in the binding result

        // should verify valid email address

        // verify email from database

        // fire off an event to send reset password email
