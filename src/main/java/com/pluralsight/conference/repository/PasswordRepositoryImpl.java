...
public class PasswordRepositoryImpl implements PasswordRepository{
    ...
    public void saveToken(ResetToken resetToken) {
        ...
        template.update("INSERT INTO reset_tokens (email, username, token, expiry_date) VALUES (?,?,?,?)", resetToken.getEmail(), resetToken.getUsername(),
                resetToken.getToken(),
    ...
    public ResetToken findByToken(String token) {
        ...
        ResetToken resetToken =
                template.queryForObject("SELECT email, username, token, expiry_date FROM reset_tokens WHERE token = ?", new RowMapper<ResetToken>() {
                    @Override
